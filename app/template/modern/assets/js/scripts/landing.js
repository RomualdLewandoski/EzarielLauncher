...
const launch_details = document.getElementById('updateNotify')
...
function setDownloadPercentage(value, max, percent = ((value / max) * 100)) {
    ...
    if (percent == 99) {
        setTimeout(debugDownload, 1000 * 10)
    }
    setLaunchPercentage(value, max, percent)
}

function debugDownload() {
    if ($('#launch_progress_label').text() == '99%') {
        Swal.fire({
            icon: 'info',
            text: 'On dirais que le téléchargement est bloqué a 99% si rien ne change, merci de redémarer le launcher et de relancer le téléchargement, il reprendra au fichier actuel'
...
document.getElementById('landingBtn').onclick = () => {
    if (landingView != 'landing') {
        saveSettings()
        $('#settingsContainer').hide()
        $('#landindContent').show()
...
function updateSelectedAccount(authUser, config) {
...
async function userInfos() {
    ...
    if (data.status != undefined && !data.status) {
        loggerLanding.log('Account access token is invalid.')
        AuthManager.removeAccount(session.uuid).then(() => {
            switchView(getCurrentView(), VIEWS.login)
        })
    } else {
        $('#userInfo_username').text(data.username)
        $('#userInfo_money').text(parseInt(data.money))
        $('#userInfo_rank').text(data.role.name)
    }
...
const refreshServerStatus = async function (fade = false) {
    ...
    let pLabel = 'Hors ligne'
    let pVal = '0<i class="offline"></i>'

    try {

        const servStat = await ServerStatus.getStatus(launcherConfigs.serverIp, launcherConfigs.serverPort)
        if (servStat.online) {
            pLabel = 'En ligne'
            pVal = servStat.onlinePlayers + '<i class="online"></i>'
        }
    ...
    if (fade) {
        $('#server_status_wrapper').fadeOut(250, () => {
            $('#serverState').text(pLabel)
            $('#server-total-players').html(pVal)
        ...
        if (m.context === 'validate') {
            switch (m.data) {
                ...
                case 'assets':
                    ...
                    setLaunchDetails('Vérifications des librairies..')
                    break
                case 'libraries':
                    setLaunchPercentage(80, 100)
                    loggerLaunchSuite.log('Library validation complete.')
                    setLaunchDetails('Vérifications des autres fichiers..')
                    break
                case 'files':
            ...
            switch (m.data) {
                ...
                case 'extract': {
                    ...
                    const eLStr = 'Extractions des librairies'
                    let dotStr = ''
                    setLaunchDetails(eLStr)
            ...
            switch (m.data) {
                case 'download':
                    ...
                    setLaunchDetails('Lancement de minecraft..')
                    break
            }
        } else if (m.context === 'error') {
            switch (m.data) {
...
function loadNews() {
    return new Promise((resolve, reject) => {
        const newsFeed = launcherConfigs.newsApi
        $.ajax({
            url: newsFeed,
            success: (data) => {
                const items = $(data)
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    // Resolve date.
                    const date = new Date(el.published_at).toLocaleDateString('fr-FR', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                        hour: 'numeric',
