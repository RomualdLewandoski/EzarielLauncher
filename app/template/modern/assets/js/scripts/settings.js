...
$('.optionHead').click((event) => {
    let current = $(event.currentTarget)
    let icon = current.children().last()
    icon.children(0).toggleClass('fa-angle-down fa-angle-up')
    icon.toggleC
    let panel = current.next('.optionContent')
    panel.slideToggle()
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
                ...
                setOverlayContent(
                    'Attention<br>',
                    'Vous devrez vous reconnecter apres cette action.<br><br>Etes vous sur de vouloir continuer ?',
                    'Oui',
                    'Non'
                )
                setOverlayHandler(() => {
...
$('#ratioSelector').change(() => {
    let ratio = $('#ratioSelector').val()
    if (ratio != '') {
        let ratioArr = ratio.split('x')
        $('#settingsGameWidth').val(ratioArr[0])
        $('#settingsGameHeight').val(ratioArr[1])
    }
})
...
settingsMaxRAMRange.setAttribute('min', '1')
settingsMinRAMRange.setAttribute('max', SETTINGS_MAX_MEMORY)
settingsMinRAMRange.setAttribute('min', '1')
